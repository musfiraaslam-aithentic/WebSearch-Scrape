...
def process_queries_file(filename="queries.txt", output_dir="results", skip_existing=True):
    ...
    instructions = (
        ...
        "ONLY give me the model name and specification summary, nothing else."
        "Do not include How the Information Was Obtained"
    )

    total = len(queries)
    for i, raw_query in enumerate(queries, start=1):
        query = f"Search for {raw_query} and extract information about Model Name and Specifications."
